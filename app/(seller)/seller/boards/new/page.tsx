...
import { Plus, Minus, Trash2, ImagePlus, Sparkles } from "lucide-react";
...
export default function NewBoardPage() {
  ...
  const [prizeTiers, setPrizeTiers] = useState<PrizeTier[]>([
    { id: "1", tier: "1등", name: "", quantity: 1, color: TIER_COLORS[0].color, images: [], imagePreview: [] },
    { id: "2", tier: "2등", name: "", quantity: 3, color: TIER_COLORS[1].color, images: [], imagePreview: [] },
    { id: "3", tier: "3등", name: "", quantity: 10, color: TIER_COLORS[2].color, images: [], imagePreview: [] },
  ]);
      ...
      prizeTiers.map((t) =>
        t.id === id
          ? {
              ...
              imagePreview: [...t.imagePreview, ...previews],
            }
          : t
      )
    );
  ...
  const removeImage = (tierId: string, imageIndex: number) => {
    setPrizeTiers(
      prizeTiers.map((t) => {
        if (t.id !== tierId) return t;
        const newImages = [...t.images];
        const newPreviews = [...t.imagePreview];
        newImages.splice(imageIndex, 1);
        newPreviews.splice(imageIndex, 1);
        return { ...t, images: newImages, imagePreview: newPreviews };
      })
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < prizeTiers.length; i++) {
        ...
        // 이미지 업로드
        for (const image of tier.images) {
          const fileName = `${board.id}/${tier.id}/${Date.now()}-${image.name}`;
      ...
      // 4. 오버레이 상태 초기화
      await supabase.from("overlay_states").insert({
        board_id: board.id,
      });

      toast.success("쿠지판이 생성되었습니다!");
      router.push(`/seller/boards/${board.id}`);
    } catch (error) {
      console.error(error);
      toast.error("쿠지판 생성에 실패했습니다");
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white">새 쿠지판 만들기</h1>
        <p className="text-gray-400 mt-1">경품과 당첨 개수를 설정하세요</p>
      </div>
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1 flex-wrap">
                        {tier.imagePreview.map((preview, i) => (
